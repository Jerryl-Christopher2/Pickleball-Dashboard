...
def _get_worksheet(tab_name):
    ...
    Cached as a *resource*, not just the data: looking a worksheet up by
    name is itself a Sheets API call, and Streamlit reruns this whole
    script on every click/interaction — without caching this, init_storage()
    alone would re-hit the API for all 9 tabs on every single rerun, which
    is exactly what was blowing through Google's per-minute quota."""
    sh = _spreadsheet()
    try:
...
def _read_tab(tab_name):
    ...
    self-healing CSV loaders. Cached so clicking around the app doesn't
    re-hit the Sheets API on every single rerun — a save always clears
    this cache immediately (see _write_tab), so your own changes are never
    stale; only another concurrent user's changes could lag by up to the
    TTL."""
...
def inject_apple_design_system():
    """A single CSS pass giving the whole app a refined, Apple-inspired
    look: system font stack, generous whitespace, a light neutral
    background with white "surface" cards, one restrained accent color,
    consistent rounded corners, soft shadows and translucency, smooth
    hover/focus transitions, and a calm, native-feeling set of controls.
    Every page gets this automatically — it's one CSS pass, not a
    page-by-page rewrite, so nothing about how the app works changes."""
    st.markdown(
        """
        <style>
        :root {
            ...
            --apple-green: #34c759;
            --apple-green-tint: rgba(52, 199, 89, 0.1);
            --apple-red: #ff3b30;
            --apple-red-tint: rgba(255, 59, 48, 0.08);
            --apple-orange: #ff9500;
            --apple-orange-tint: rgba(255, 149, 0, 0.1);
            ...
            --apple-shadow-md: 0 4px 20px rgba(0, 0, 0, 0.06);
            --apple-shadow-lg: 0 12px 40px rgba(0, 0, 0, 0.10);

            /* ---- Motion ---- */
            --apple-ease: cubic-bezier(0.25, 0.46, 0.45, 0.94);
            --apple-duration: 0.18s;
        }
        ...
        /* Translucent, blurred top bar -- Apple's frosted-glass chrome */
        div[data-testid="stHeader"] {
            background: var(--apple-surface-translucent) !important;
        ...
        /* Gentle content fade-in on every rerun -- quiet, not distracting */
        @keyframes apple-fade-in {
            from { opacity: 0; transform: translateY(4px); }
            to { opacity: 1; transform: translateY(0); }
        }
        section.main div.block-container { animation: apple-fade-in 0.28s var(--apple-ease); }
        ...
        p, li, .stMarkdown { color: var(--apple-text); line-height: 1.47; }
        ...
        /* Sidebar nav radio -> segmented / iOS-settings-style pill list */
        ...
        div[data-testid="stButton"] > button, div[data-testid="stFormSubmitButton"] > button {
            ...
            color: var(--apple-text);
            transition: all var(--apple-duration) var(--apple-ease) !important;
        ...
        div[data-testid="stNumberInput"] input:focus-visible {
            ...
            box-shadow: 0 0 0 4px var(--apple-blue-tint) !important;
        }

        /* ---- Inputs, selects, dropdown popovers ------------------------ */
        ...
        div[data-testid="stMultiSelect"] > div > div {
            ...
            transition: border-color var(--apple-duration) var(--apple-ease),
                        box-shadow var(--apple-duration) var(--apple-ease) !important;
        }
        ...
        /* Dropdown option list (BaseWeb popover, portalled to body) */
        div[data-baseweb="popover"] div[data-baseweb="menu"],
        ul[role="listbox"] {
            border-radius: var(--apple-radius-md) !important;
            border: 1px solid var(--apple-border-light) !important;
            box-shadow: var(--apple-shadow-lg) !important;
            background: var(--apple-surface) !important;
            overflow: hidden;
        }
        li[role="option"]:hover, li[aria-selected="true"] {
            background: var(--apple-blue-tint) !important;
        }

        /* Checkboxes */
        div[data-testid="stCheckbox"] label { gap: 0.5rem; }

        /* Non-sidebar radios (in-page choices) -> compact segmented pills */
        ...
        /* ---- Cards: metrics, expanders, dataframes, alerts ------------- */
        div[data-testid="stMetric"] {
            ...
            padding: 1rem 1.2rem;
            box-shadow: var(--apple-shadow-sm);
            transition: box-shadow var(--apple-duration) var(--apple-ease);
        }
        ...
        div[data-testid="stExpander"] summary:hover { background: rgba(0, 0, 0, 0.02); }
        ...
        hr { border-color: var(--apple-border-light) !important; margin: 1.75rem 0 !important; }

        /* ---- Hero header ------------------------------------------------ */
        .apple-hero { padding: 0.25rem 0 0.5rem 0; margin-bottom: 0.5rem; }
        .apple-hero h1 { font-size: 2.1rem !important; margin-bottom: 0.15rem !important; }
        .apple-hero .apple-hero-subtitle {
...
st.markdown(
    ...
    <div class="apple-hero">
        <h1>🏓 Pickleball Rankings</h1>
        <div class="apple-hero-subtitle">Season {current_season_number()} in progress · synced to Google Sheets</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
nav_choice = st.sidebar.radio("Navigate", list(NAV_PAGES.keys()), label_visibility="collapsed")
page = NAV_PAGES[nav_choice]
    ...
    if seasons_df.empty:
        ...
        for _, srow in seasons_df.sort_values("season_id", ascending=False).iterrows():
            ...
            with st.expander(f"🏆 {srow['label']}  —  {srow['start_date']} to {srow['end_date']}", expanded=False):
                ...
                sp["Win %"] = (sp["wins"] / sp["Games"].replace(0, float("nan")) * 100).round(1)
